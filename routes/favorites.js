 ...
 * Atomically toggles bookmark. Safe against rapid clicks and duplicate keys.
 */
router.post('/toggle', authMiddleware, async (req, res) => {
    try {
        ...
        // Try to remove first (un-bookmark)
        ...
        if (removed) {
            ...
            // Try to insert (bookmark). Use upsert to safely handle duplicate key
            // from rapid double-taps — $setOnInsert means no write if already exists.
            const result = await Favorite.updateOne(
                { userId, fileId: fileObjectId },
                { $setOnInsert: { userId, fileId: fileObjectId } },
                { upsert: true }
            );

            // If nothing was upserted, a concurrent request already inserted it;
            // treat as already bookmarked.
            isBookmarked = true;
        }
        ...
        // Final safety net for any unexpected duplicate key errors
        if (err.code === 11000) {
            return res.json({
                success: true,
                data: { bookmarked: true, isBookmarked: true, fileId: req.body.fileId },
            });
        }
