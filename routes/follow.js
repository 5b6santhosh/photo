    ...
    try {
        ...
        await session.withTransaction(async () => {
            const removed = await Following.findOneAndDelete(
                { follower: followerObjectId, following: targetObjectId },
                { session }
            );
            ...
            if (removed) {
                ...
                await User.findByIdAndUpdate(
                    targetObjectId,
                    // Pipeline update: floor at 0, never go negative
                    [{ $set: { followersCount: { $max: [0, { $subtract: ['$followersCount', 1] }] } } }],
                    { session }
                );
            } else {
                // ── Follow — upsert so duplicate key is a safe no-op ─────────────
                const result = await Following.updateOne(
                    { follower: followerObjectId, following: targetObjectId },
                    { $setOnInsert: { follower: followerObjectId, following: targetObjectId } },
                    { upsert: true, session }
                );

                // Only increment if we actually inserted a new document
        ...
        // Read authoritative count AFTER transaction commits
